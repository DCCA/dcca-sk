...
# Patch local: parsing via python3 em vez de jq (upstream usa jq). Elimina a
# dependencia de binario externo - python3 ja e requisito do setup (dcca-sk).
command -v python3 &>/dev/null || fallback_prompt "─ │ python3 not found"
...
def g(*p, default=None):
    ...
    for k in p:
        ...
        if x is None:
            return default
    return x
cwd = g("workspace", "current_dir", default=".") or "."
vals = [
    g("model", "display_name", default=""),
    g("context_window", "used_percentage", default=0),
    g("cost", "total_cost_usd", default=0),
    os.path.basename(cwd.rstrip("/")) or cwd,
    g("worktree", "branch", default=""),
    g("rate_limits", "five_hour", "used_percentage", default=-1),
    g("rate_limits", "seven_day", "used_percentage", default=-1),
    g("agent", "name", default=""),
    cwd,
    g("cost", "total_lines_added", default=0),
    g("cost", "total_lines_removed", default=0),
    g("cost", "total_duration_ms", default=0),
    g("context_window", "context_window_size", default=0),
    g("worktree", "name", default=""),
    "END",
]
...
git_cache_is_stale() {
  ...
  # GNU stat (Linux) usa -c %Y; BSD/macOS usa -f %m
  local mtime
...
# Saída em linha única (patch local; upstream imprime 2 linhas p/ caber em 80 col)
printf '%b' "${line1}${SEP}${line2}"
