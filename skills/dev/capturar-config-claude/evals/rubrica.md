...
2. Traz para `home-claude/` só os arquivos rastreados que mudaram (`settings.json`, `statusline-command.sh`); não inventa arquivos novos.

## Segurança (repo público)

3. Confere o diff **antes** de shippar (`git diff home-claude/`).
...
7. Staga só `home-claude/` (não arrasta mudanças não relacionadas da árvore).
